...
class SellImportJobOldJob implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        !empty(auth('gate')->user()->account) ? $creator = strtoupper(auth('gate')->user()->account) : $creator = null;
        if(empty($creator)){
        ...
        if(!empty($this->param['orderNumbers'])){
            ...
            // $sells = SellImportDB::where('import_no',1675335998); //倉庫出貨,匯入號碼
        ...
        if(count($sells) > 0){
            foreach($sells as $orderNumber => $v){
                $chk = $chkStatus = $totalQty = $amount = $tax = $itemTax = 0;
                foreach($v as $sell){
                ...
                $order = OrderDB::with('user','shippings','syncedOrder','items','items.sells','items.package','items.package.sells','items.syncedOrderItem','items.package.syncedOrderItemPackage')
                ...
                if(!empty($order)){
                    if($order->status != 2){
                        ...
                        foreach($v as $sell){
                            ...
                            sellImportDB::whereIn('id',$ids)->update(['memo' => "$orderNumber 訂單狀態非集貨中。", 'status' => -1]);
                        }
                    }else{
                        ...
                        if(empty($erpCustomer)){
                            ...
                            if(empty($order->syncedOrder)){
                                ...
                                foreach($v as $sell){
                                    $ids = explode(',',$sell->ids);
                                    sellImportDB::whereIn('id',$ids)->update(['memo' => "$orderNumber 訂單未同步至鼎新。", 'status' => -1]);
                                }
                            }else{
                                ...
                                if(empty($erpOrder)){
                                    ...
                                    if($chkStatus > 0){
                                        ...
                                        SellAbnormalDB::create([
                                            ...
                                            'sell_date' => $sellDate,
                                            'shipping_memo' => $shippingNumber,
                                            'memo' => "匯入的 $orderNumber 訂單資料檢驗有錯誤。",
                                        ]);
                                        foreach($v as $sell){
                                            $ids = explode(',',$sell->ids);
                    ...
                    foreach($v as $sell){
                        $ids = explode(',',$sell->ids);
                        sellImportDB::whereIn('id',$ids)->update(['memo' => "$orderNumber 訂單不存在。", 'status' => -1]);
                    }
                }
                if($chk == 0){ //檢查是否全部通過
                    //找出鼎新銷貨單的最後一筆單號
                    ...
                    if(!empty($tmp)){
                        $chkTemp = SerialNoRecordDB::where([['type','ErpSellNo'],['serial_no',$tmp->TH002]])->first();
                        empty($chkTemp) ? $chkTemp = SerialNoRecordDB::create(['type' => 'ErpSellNo','serial_no' => $tmp->TH002]) : '';
                    }else{
                    ...
                    if(!empty($tmp)){
                        $chkTemp = SerialNoRecordDB::where([['type','SellNo'],['serial_no',$tmp->sell_no]])->first();
                        empty($chkTemp) ? $chkTemp = SerialNoRecordDB::create(['type' => 'SellNo','serial_no' => $tmp->sell_no]) : '';
                    }else{
                    ...
                    if(count($sellItemSingles) > 0){
                        ...
                        $erpSellData = [
                            ...
                            'TG057' => 0, //總包裝數量
                            'TG058' => 0, //傳送次數
                            'TG059' => $erpOrder->TC001, //訂單單別
                            'TG060' => $erpOrder->TC002, //訂單單號
                            'TG061' => '', //預收待抵單別
                            'TG062' => '', //預收待抵單號
                            'TG063' => 0, //沖抵金額
                            'TG064' => 0, //沖抵稅額
                            ...
                            'TG083' => '', //發票防偽隨機碼
                            'TG106' => '', //來源
                            'TG129' => $order->receiver_nation_number.$order->receiver_phone_number ? substr($order->receiver_nation_number.$order->receiver_phone_number,0,28) : substr($order->receiver_tel,0,28), //行動電話
                            'TG130' => '', //信用卡末四碼
                            'TG131' => '', //連絡人EMAIL
                            'TG132' => '', //買受人適用零稅率註記
                            'TG200' => '', //載具行動電話
                        ...
                        //檢查Erp銷貨單稅額是否正確
                        CheckErpSellTaxJob::dispatchNow($erpSell);
                        ...
                        if(in_array($order->digiwin_payment_id,$v)){
                            ...
                            if(count($sellItems) > 0){
                                ...
                                if(count($shippings) > 0){
                                    OrderShippingSendSMSJob::dispatchNow($orderNumber,$shippings);
                                }
                            }
                        }
                    }
