...
<div class="content-wrapper">
    <div class="content-header">
        <div class="container-fluid">
            ...
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h1 class="m-0 text-dark"><b>訂單匯入異常列表</b></h1>
                </div>
                <div class="col-sm-6">
                    <ol class="breadcrumb float-sm-right">
                        <li class="breadcrumb-item"><a href="{{ route('gate.dashboard') }}">中繼管理系統</a></li>
                        <li class="breadcrumb-item active"><a href="{{ url('orderAbnormals') }}">訂單匯入異常列表</a></li>
                    ...
                    <div class="card">
                        <div class="card-header">
                            <div class="float-left">
                                ...
                                <form action="{{ route('gate.orderImportAbnormals.deleteAll') }}" method="POST">
                                    @csrf
                                    <button type="button" class="del-btn btn btn-danger">一鍵處理</button>
                                </form>
                                @endif
                            </div>
                            <div class="float-right">
                                <div class="input-group input-group-sm align-middle align-items-middle">
                                    <span class="badge badge-purple text-lg mr-2">總筆數：{{ !empty($orderAbnormals) ? number_format($orderAbnormals->total()) : 0 }}</span>
                                </div>
                            </div>
                        </div>
                        <div class="card-body">
                            @if(count($orderAbnormals) > 0)
                            <table class="table table-hover table-sm">
                                <thead>
                                    <tr>
                                        <th class="text-left text-sm" width="5%">匯入類別</th>
                                        <th class="text-left text-sm" width="10%">客戶訂單號碼</th>
                                        <th class="text-left text-sm" width="10%">貨號</th>
                                        <th class="text-right text-sm" width="5%">商品<br>數量</th>
                                        <th class="text-right text-sm" width="5%">商品<br>單價</th>
                                        <th class="text-left text-sm" width="30%">異常原因</th>
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        <td class="text-left text-sm align-middle">{{ $orderAbnormal->type }}</td>
                                        <td class="text-left text-sm align-middle">{{ $orderAbnormal->partner_order_number }}</td>
                                        <td class="text-left text-sm align-middle">{{ $orderAbnormal->sku }}</td>
                                        <td class="text-right text-sm align-middle">{{ $orderAbnormal->quantity }}</td>
                                        <td class="text-right text-sm align-middle">{{ $orderAbnormal->price }}</td>
                                        <td class="text-left text-sm align-middle">{{ $orderAbnormal->memo }}</td>
                                        <td class="text-center text-sm align-middle">{{ !empty($orderAbnormal->chk_date) ? explode(' ',$orderAbnormal->chk_date)[0] : null }}<br>{{ !empty($orderAbnormal->chk_date) ? explode(' ',$orderAbnormal->chk_date)[1] : null }}</td>
                                        <td class="text-left text-sm align-middle">{{ $orderAbnormal->admin_name }}</td>
                                        @if(in_array($menuCode.'M',explode(',',Auth::user()->power)))
                                        <td class="text-center align-middle">
                                            @if($orderAbnormal->is_chk == 0)
                                            <form action="{{ route('gate.orderImportAbnormals.update', $orderAbnormal->id) }}" method="POST">
                                                @csrf
                        ...
                        <div class="card-footer bg-white">
                            <div class="float-left">
                                <span class="badge badge-primary text-lg ml-1">總筆數：{{ !empty($orderAbnormals) ? number_format($orderAbnormals->total()) : 0 }}</span>
                            </div>
                            <div class="float-right">
                                @if(isset($appends))
                                {{ $orderAbnormals->appends($appends)->render() }}
                                @else
                                {{ $orderAbnormals->render() }}
                                @endif
                            </div>
                        </div>
                    </div>
...
<script>
    (function($) {
        ...
        $('.del-btn').click(function (e) {
            if(confirm('請確認是否要一次處理全部資料?')){
                $(this).parents('form').append($('<input type="hidden" name="is_chk" value="all">'));
                $(this).parents('form').submit();
            };
        });
