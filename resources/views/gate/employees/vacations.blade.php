...
<div class="content-wrapper">
    <div class="content-header">
        <div class="container-fluid">
            ...
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h1 class="m-0 text-dark"><b>員工請假紀錄</b></h1>
                </div>
                <div class="col-sm-6">
                    <ol class="breadcrumb float-sm-right">
                        <li class="breadcrumb-item"><a href="{{ route('gate.dashboard') }}">中繼管理系統</a></li>
                        <li class="breadcrumb-item active"><a href="{{ url('vacations') }}">員工請假紀錄</a></li>
                    ...
                    <div class="card">
                        ...
                        <div class="card-body">
                            <table class="table table-hover table-sm">
                                <thead>
                                    <tr>
                                        <th class="text-left" width="20%">工號</th>
                                        <th class="text-left" width="20%">姓名</th>
                                        <th class="text-left" width="20%">請假類別</th>
                                        <th class="text-left" width="15%">開始時間</th>
                                        <th class="text-left" width="15%">結束時間</th>
                                        <th class="text-center" width="10%">時長(hr)</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    @foreach ($vacations as $vacation)
                                    <tr>
                                        <td class="text-left align-middle">{{ $vacation->employee_no }}</td>
                                        <td class="text-left align-middle">{{ !empty($vacation->employee) ? $vacation->employee->name : null }}</td>
                                        <td class="text-left align-middle">{{ $vacation->type }}</td>
                                        <td class="text-left align-middle">{{ $vacation->start_time }}</td>
                                        <td class="text-left align-middle">{{ $vacation->end_time }}</td>
                                        <td class="text-center align-middle">{{ $vacation->duration }}</td>
                                    </tr>
                                    @endforeach
                                </tbody>
                            </table>
                        ...
                        <div class="card-footer bg-white">
                            <div class="float-left">
                                <span class="badge badge-primary text-lg ml-1">總筆數：{{ !empty($vacations) ? number_format($vacations->total()) : 0 }}</span>
                            </div>
                            <div class="float-right">
                                @if(isset($appends))
                                {{ !empty($vacations) ? (!empty($appends) ? $vacations->appends($appends)->render() : '') : '' }}
                                @else
                                {{ $vacations->render() }}
                                @endif
                            </div>
    ...
    <div class="modal-dialog modal-xl" style="max-width: 60%;">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form  id="importForm" action="{{ url('vacations/import') }}" method="POST" enctype="multipart/form-data">
                    @csrf
                ...
                <div>
                    <span class="text-danger">注意! 請選擇正確的檔案並填寫正確的資料格式匯入，否則將造成資料錯誤，若不確定格式，請參考 <a href="./sample/員工請假資料範本.xlsx" target="_blank">員工請假資料範本</a> ，製作正確的檔案。</span>
                </div>
            </div>
        </div>
    </div>
