...
<div class="content-wrapper">
    <div class="content-header">
        <div class="container-fluid">
            ...
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h1 class="m-0 text-dark"><b>員工資料</b></h1>
                </div>
                <div class="col-sm-6">
                    <ol class="breadcrumb float-sm-right">
                        <li class="breadcrumb-item"><a href="{{ route('gate.dashboard') }}">中繼管理系統</a></li>
                        <li class="breadcrumb-item active"><a href="{{ url('employees') }}">員工資料</a></li>
                ...
                <div class="col-12">
                    <div class="card">
                            ...
                            <table class="table table-hover table-sm">
                                <thead>
                                    <tr>
                                        ...
                                        <th class="text-left" width="20%">部門</th>
                                        <th class="text-left" width="20%">職稱</th>
                                        <th class="text-center" width="10%">到職日</th>
                                        <th class="text-center" width="10%">離職日</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    @foreach ($employees as $employee)
                                    <tr>
                                        <td class="text-left align-middle">{{ $employee->employee_no }}</td>
                                        <td class="text-left align-middle">{{ $employee->name }}</td>
                                        <td class="text-left align-middle">{{ $employee->department }}</td>
                                        <td class="text-left align-middle">{{ $employee->title }}</td>
                                        <td class="text-center align-middle">{{ $employee->onduty_date }}</td>
                                        <td class="text-center align-middle">{{ $employee->leave_date }}</td>
                                    </tr>
                                    @endforeach
                                </tbody>
                            </table>
                        ...
                        <div class="card-footer bg-white">
                            <div class="float-left">
                                <span class="badge badge-primary text-lg ml-1">總筆數：{{ !empty($employees) ? number_format(count($employees)) : 0 }}</span>
                            </div>
                            <div class="float-right">
                                @if(isset($appends))
                                {{ !empty($employees) ? (!empty($appends) ? $employees->appends($appends)->render() : '') : '' }}
                                @else
                                {{ !empty($employees) ? $employees->render() : '' }}
                                @endif
                            </div>
                        </div>
                    </div>
    ...
    <div class="modal-dialog modal-xl" style="max-width: 60%;">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form  id="importForm" action="{{ url('employees/import') }}" method="POST" enctype="multipart/form-data">
                    @csrf
                ...
                <div>
                    <span class="text-danger" id="importModalNotice">注意! 請選擇正確的檔案並填寫正確的資料格式匯入，否則將造成資料錯誤，若不確定格式，請參考 <a href="./sample/員工資料範本.xlsx" target="_blank">員工資料範本</a> ，製作正確的檔案。</span>
                </div>
            </div>
        </div>
    </div>
