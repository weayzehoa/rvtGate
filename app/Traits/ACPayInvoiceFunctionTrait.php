...
trait ACPayInvoiceFunctionTrait
{
    public function acpayAllowance($order, $param)
    {
        $totalAmt = 0;
                ...
                if($chkRemain == 0){
                    ...
                    if(in_array($order->digiwin_payment_id,['AC0001','AC000101','AC000102','AC000103'])){
                        ...
                        for($i=0;$i<count($items);$i++){
                            ...
                            $totalAmt += $items[$i]['quantity'] * $items[$i]['price'];
                        }
                    }
                    //ezPay參數
    ...
    public function acpayCreate($order, $param, $newOrderNumber = null)
    {
        if(!empty($order)){
    ...
    public function acpayCancel($orders,$type,$reason)
    {
    ...
    public function acpayPost($postDataArray)
    {
        ...
        if (phpversion() > 7) {
            //php 7 以上版本加密
            $postData = trim(bin2hex(openssl_encrypt($this->acAddPadding($postDataStr),'AES-256-CBC', $key, OPENSSL_RAW_DATA | OPENSSL_ZERO_PADDING, $iv)));
        } else {
            //php 7 之前版本加密
            $postData = trim(bin2hex(mcrypt_encrypt(MCRYPT_RIJNDAEL_128, $key,$this->acAddPadding($postDataStr), MCRYPT_MODE_CBC, $iv)));
        }
    ...
    private function acAddPadding($string, $blocksize = 16) {
        $len = strlen($string);
