...
class ACErpOrder extends Model
{
    use HasFactory;
    ...
    public function items(){
        return $this->hasMany(ACErpOrderItemDB::class,'TD002','TC002')->where('TD016','!=','y');
    }

    public function customer(){//不能使用belongsTo, 也不能用with, 否則找不到資料, 只能直接使用 order->customer
        return $this->hasOne(ACErpCustomerDB::class,'MA001','TC004');
    }
}
