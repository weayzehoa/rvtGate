...
class DirectShipmentSellImportJob implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        if($this->param['type'] == 'directShip'){
            $directShipment = 1;
            $sells = $sells->groupBy('order_number','shipping_number','digiwin_no')->orderBy('id','desc')->get();
        }else{
            $directShipment = 0;
        ...
        if(count($sells) > 0){
            foreach($sells as $orderNumber => $v){
                ...
                $order = OrderDB::with('acOrder','user','shippings','syncedOrder','items','items.sells','items.package','items.package.sells','items.syncedOrderItem','items.package.syncedOrderItemPackage')
                                                                ...
                                                                if(count($lists) > 0){
                                                                    foreach($lists as $list){
                                                                        foreach($item->package as $package){
                                                                            $packageSku = $package->sku;
                                                                            //如果有轉換貨號 要反推回去
                                                                            if(!empty($package->order_digiwin_no)){
                                                                                $pmTemp = ProductModelDB::where('digiwin_no',$package->order_digiwin_no)->first();
                                                                                $packageSku = $pmTemp->sku;
                                                                            }
                                                                            if($packageSku == $list->sku && !empty($package->syncedOrderItemPackage)){
                                                                                $chkProduct++;
                        ...
                        if(in_array($order->digiwin_payment_id,$v)){
                            ...
                            if(count($sellItems) > 0){
                                ...
                                if(count($shippings) > 0){
                                    OrderShippingSendSMSJob::dispatch($orderNumber,$shippings);
                                }
                            }
                        }
                    }
