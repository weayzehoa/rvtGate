...
use App\Imports\EmployeeVacationFileImport;

use Carbon\Carbon;

class EmployeeVacationFileImportJob implements ShouldQueue
{
    ...
    {
        $param = $this->param;
        $result = Excel::toArray(new EmployeeVacationFileImport, $param['filename']); //0代表第一個sheet
        if(count($result) == 1){
            if($result[0][0][0] == '審批編號' && count($result[0][0]) == 20){
                $results = $result[0];
                for($i=1;$i<count($results);$i++){
                    if($this->chkData($results[$i]) == true){
                        ...
                        if(!empty($employeeNo) && $status == '同意'){
                            $employeeVacation = EmployeeVacationDB::where('approval_no',$approvalNo)->first();
                            if(!empty($employeeVacation)){
