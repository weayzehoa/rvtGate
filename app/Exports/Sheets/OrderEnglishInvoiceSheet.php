...
class OrderEnglishInvoiceSheet implements FromCollection,WithStrictNullComparison,WithStyles,WithTitle,WithColumnWidths
{
        ...
        if(!empty($orders)){
            ...
            foreach ($orders as $order) {
                ...
                foreach($order->items as $item){
                    $productId = $item->model->product_id;
                    $tmp = ProductLangDB::find($productId);
                    $tmp ? $item->product_name = $tmp->name : '';
                ...
                if($order->shipping_fee > 0){
                    $data[$x+$i] = ['Z','Freight','1', round($order->shipping_fee / $USD, 2), round($order->shipping_fee / $USD, 2)];
                ...
                if($order->spend_point > 0){
                    $data[$x+$i] = ['','Shopping Coin','1',-round($order->spend_point / $USD, 2),-round($order->spend_point / $USD, 2)];
                ...
                if($order->parcel_tax > 0){
                    $data[$x+$i] = ['Y','Parcel Tax','1',round($order->parcel_tax / $USD, 2),round($order->parcel_tax / $USD, 2)];
                    $totalAmount += round($order->parcel_tax / $USD, 2);
                    $totalQuantity += 1;
                    $i++;
                }
                ...
                if($order->discount != 0){
                    if($order->discount > 0){
                        $data[$x+$i] = ['','Discount','1',-round($order->discount / $USD , 2),-round($order->discount / $USD , 2)];
