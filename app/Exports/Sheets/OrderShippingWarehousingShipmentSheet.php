...
class OrderShippingWarehousingShipmentSheet implements FromCollection,WithStrictNullComparison,WithStyles,WithTitle,WithHeadings,ShouldAutoSize
{
    ...
    */
    public function collection()
    {
        ...
        if(!empty($orderIds)){
            $items = OrderItemDB::join($orderTable,$orderTable.'.id',$orderItemTable.'.order_id')
                ...
                ])->groupBy('sku','book_shipping_date','shipping_memo')->orderBy('sku','asc')->get();
    ...
    public function styles(Worksheet $sheet)
    {
        ...
        for($i=1; $i<$count; $i++){
            $sheet->getStyle("A$i:K$i")->getBorders()->getAllBorders() //框線
