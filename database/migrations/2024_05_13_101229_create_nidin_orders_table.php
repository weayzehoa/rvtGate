...
class CreateNidinOrdersTable extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        if (env('DB_MIGRATE_NIDIN_ORDERS_TABLE')) {
            Schema::create('nidin_orders', function (Blueprint $table) {
                $table->id();
                $table->string('merchant_no')->nullable()->comment('ac特店代號');
                $table->string('serial_no')->nullable()->comment('ac交易序號');
                ...
                $table->integer('discount')->nullable()->comment('訂單折扣金額');
    ...
    {
        if (env('DB_MIGRATE_NIDIN_ORDERS_TABLE')) {
            Schema::dropIfExists('nidin_orders');
        }
    }
}
